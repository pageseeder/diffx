...
// methods ------------------------------------------------------------------------------------
  ...
  public void process(DiffXFormatter formatter) throws IOException {
    ...
    while (i < this.length1 && j < this.length2) {
      ...
      if (this.matrix.isGreaterX(i, j)) {
        ...
        if (this.estate.okInsert(e1) && !this.estate.hasPriorityOver(e2, e1)) {
          if (DEBUG) System.err.print("["+i+","+j+"]->["+(i+1)+","+j+"] >i +"+ShortStringFormatter.toShortString(e1));
          formatter.insert(e1);
        ...
        // if we can format checking at the stack, let's do it
          ...
          if (DEBUG) System.err.print("["+i+","+j+"]->["+(i+1)+","+(j+1)+"] >f "+ShortStringFormatter.toShortString(e1));
          formatter.format(e1);
        ...
        // go counter current and delete
          ...
          if (DEBUG) System.err.print("["+i+","+j+"]->["+i+","+(j+1)+"] >d -"+ShortStringFormatter.toShortString(e2));
          formatter.delete(e2);
      ...
      if (DEBUG) System.err.println("    stack:"+this.estate.currentChange()+ShortStringFormatter.toShortString(this.estate.current()));
    }

    // finish off the events from the first sequence
    while (i < this.length1) {
      if (DEBUG) System.err.println("["+i+","+j+"]->["+(i+1)+","+j+"] _i -"+ShortStringFormatter.toShortString(this.sequence1.getEvent(i)));
    ...
    while (j < this.length2) {
      if (DEBUG) System.err.println("["+i+","+j+"]->["+i+","+(j+1)+"] _d -"+ShortStringFormatter.toShortString(this.sequence2.getEvent(j)));
