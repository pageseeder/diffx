...
/**
 * Wu et al. O(NP) diff implementation using a diagonal frontier.
 *
 * <p>This implementation computes the shortest edit script and emits
 * operations to the provided handler.</p>
 *
 ...
 * @since 1.3.3
 */
public final class WuAlgorithm<T> implements DiffAlgorithm<T> {
   ...
   */
  public WuAlgorithm(Equality<T> eq) {
    this.eq = eq;
  }
  ...
  private static class Instance<T> {
    ...
    private List<Snake> computePath() {
      ...
      // Find the endpoint of the furthest reaching path in diagonal k
      boolean found = false;
      for (int d = 0; d <= max; d++) {
    ...
    private List<Snake> solve(List<Vector> vectors) {
      ...
      for (int d = vectors.size() - 1; target.x() > 0 || target.y() > 0; d--) {
        ...
        if (target.isNotSame(xEnd, yEnd))
          throw new IllegalStateException("No solution for d:" + d + " k:" + k + " p:" + target
              + " V:( " + xEnd + ", " + yEnd + " )");
